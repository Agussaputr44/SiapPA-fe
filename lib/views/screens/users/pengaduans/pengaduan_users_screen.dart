...
class DropdownOption {
  ...
  const DropdownOption(
      {required this.displayText, required this.databaseValue});
}
  ...
  Future<void> _submitPengaduan() async {
    ...
    if (!_formKey.currentState!.validate()) {
      MessagesWidget.showError(
          context, "Harap lengkapi semua kolom yang wajib diisi.");
      return;
    }
    if (_selectedFiles.isEmpty) {
      MessagesWidget.showError(
          context, "Harap unggah setidaknya satu file bukti.");
      return;
    }
    ...
    for (final file in _selectedFiles) {
      ...
      if (!validExtensions.contains(fileExtension)) {
        MessagesWidget.showError(
            context, "Tipe file tidak didukung: .$fileExtension");
            ...
            child: ListView(
              children: [
                ...
                DropdownButtonFormField<DropdownOption>(
                  ...
                  validator: (value) =>
                      value == null ? 'Pilih kategori kekerasan' : null,
                ),
                const SizedBox(height: 16),
                DropdownButtonFormField<DropdownOption>(
                  ...
                  validator: (value) =>
                      value == null ? 'Pilih jenis korban' : null,
                ),
                const SizedBox(height: 16),
                TextFormField(
